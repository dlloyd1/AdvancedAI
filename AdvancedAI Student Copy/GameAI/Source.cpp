...
int main(int argc, char* args[])
{
	...
	if(InitSDL())
	{
		...
		gameScreenManager = new GameScreenManager(gRenderer, SCREEN_RAINBOW);//SCREEN_LUNAR);////SCREEN_CONWAY);//SCREEN_CHESS
...
{
	//Clear the screen.
	SDL_SetRenderDrawColor(gRenderer, 66, 220, 244, 0xFF);
...
bool Update()
{
	...
	//Add the amount of time that has passed.
	...
	{
		while (msElapsed <= MS_PER_UPDATE)
		{
			//Get the current time and calculate how much time has elapsed since the last update.
			currentTime = SDL_GetTicks();
			elapsedTime = currentTime - gOldTime;

			//Add the elapsed milliseconds to the counter.
			msElapsed += elapsedTime / 1000.0f;
		}
	}
	//Finally do the update.
	gameScreenManager->Update((float)elapsedTime, e);
