...
//Called when Turnstate is TURNSTATE_PRE
//@RETURN gamestate for player
GAMESTATE ChessPlayer::PreTurn() //CHECKMATE | CHECK | STALEMATE | NORMAL
{
...
//called on mouseclick from TakeATurn() when TurnState has progressed to Play AND GameState is NORMAL | CHECK
//@PARAM mouseclick coords
//@RETURN move success
bool ChessPlayer::MakeAMove(Vector2D boardPosition)
{
	//@NOTE: mSelectedPiecePosition becomes the [FROM] boardPosition becomes to [TO]
	...
	{
		//mCurrentMove is SELECT_A_PIECE by default before and after each turn.

		if( mCurrentMove == SELECT_A_PIECE ) //first mouse click calls MakeAMove and highlights legal move positions
		{
			...
			if( mChessBoard->currentLayout[(int)boardPosition.x][(int)boardPosition.y].colour == mTeamColour )
			{
				...
				*mSelectedPiecePosition = boardPosition;
				...
				{
					//carry out the move on a copy of the chessBoard
					Board boardToTest = *mChessBoard;
					//set testboard position at the move end = the board position at the move start (move the piece)
					boardToTest.currentLayout[moves[i].to_X][moves[i].to_Y] = boardToTest.currentLayout[moves[i].from_X][moves[i].from_Y];
					//set testboard position at move start  to be blank (piece has moved)
					boardToTest.currentLayout[moves[i].from_X][moves[i].from_Y] = BoardPiece();

					//if the move doesn't cause check, add is as a highlighted position (possible legal move)
					if(!CheckForCheck(boardToTest, mTeamColour))
						mHighlightPositions->push_back(Vector2D(moves[i].to_X, moves[i].to_Y)); 
				}

				//Change move type.
				mCurrentMove = SELECT_A_POSITION; //SELECT_A_POSITION from the legal highlighted moves
			}
			else
			{
		...
		else if( mCurrentMove == SELECT_A_POSITION ) //second mouse click calls MakeAMove and moves the piece
		{
			//Check if this position is actually another piece of the current players colour. (Not one of the highlighted positions to move to)
			//If so, switch the turn type and recall this function. (becomes a first mouse click call again)
			if( mChessBoard->currentLayout[(int)boardPosition.x][(int)boardPosition.y].colour == mTeamColour )
			{
				mCurrentMove = SELECT_A_PIECE;
				MakeAMove(boardPosition);
			}
			else //the location of the second mouse click is either a highlighted empty square | enemy piece | non-highlighted empty square
			{
				...
				{
					Vector2D highlightPos = mHighlightPositions->at(i); //quick reference the vec2d

					//one of the highighted positions was clicked on second click
					if(((int)(boardPosition.x) == (int)highlightPos.x) && ((int)(boardPosition.y) == (int)highlightPos.y)) 
					{
						//move is therefore valid
						validPosition = true;
						break;
					}
				}
				...
				if( validPosition == true )
				{
					...
					if( mChessBoard->currentLayout[(int)mSelectedPiecePosition->x][(int)mSelectedPiecePosition->y].piece == PIECE_PAWN ) //it's a pawn we're moving
					{
						//If the pawn is on its start position and it double jumps, then en'passant may be available for opponent.
						//boardPosition is the new pos, mSelectedPiecePosition is the old pos
						...
						mChessBoard->currentLayout[(int)boardPosition.x][(int)boardPosition.y].piece == PIECE_NONE )
					{
						...
						{
							cout << "Setting currentLayout from en passant" << endl;
							cout << "blanking " << (int)boardPosition.x << ", " << (int)boardPosition.y + pawnDirectionOpposite << endl;
					...
					//Move the piece into new position.
					mChessBoard->currentLayout[(int)mSelectedPiecePosition->x][(int)mSelectedPiecePosition->y].hasMoved = true; //prevent multi-castling
					...
					//Piece is in a new position. (store new pos in old pos)
					...
					{
						//Time to promote. (method will be called again from GameScreen_Chess as it has not yet returned true
